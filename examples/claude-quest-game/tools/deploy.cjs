#!/usr/bin/env node
// deploy.cjs : W6, a new Vercel project for the game (docs/game-spec.md 12.6 and 13.10). Owner: P7.
//
//   node tools/deploy.cjs --dry-run      the read-only project-name check, then prints every command a real
//                                        run would run; creates nothing, deploys nothing
//   node tools/deploy.cjs                the release: gates, site, project add, link, deploy, production e2e
//   options: --skip-gates   skip the T10 gates before site.cjs (only right after running them all yourself)
...
//
// 1. `vercel project ls` (read only, filtered and paged): pick `claude-quest`, else `claude-quest-game`;
//    abort when both exist (D15: never overwrite a project).
// 2. The T10 gates up to the local e2e, with `node tools/site.cjs --site https://<name>.vercel.app` in its place.
...
// 5. The production host (the deployment's *.vercel.app alias for the project) answers 200 without
//    authentication and serves this build (its og:url names that host). If Vercel gave the project another
//    host than predicted, site.cjs is re-run for the real host and web/ deployed again to the same project.
...
const NAMES = ['claude-quest', 'claude-quest-game'];
...
// T10, up to the local e2e (the project-name check and site.cjs sit where the spec puts them)
const GATES = [
  ['tools/proof/film.cjs'],
  ['tools/check.cjs'],
  ['tools/levels-lint.cjs'],
  ['tools/art-check.cjs'],
  ['tools/audio/game-audio.cjs'],
  ['tools/proof/engine-scenarios.cjs'],
  ['tools/proof/mech-scenarios.cjs'],
...
const AFTER_SITE = (site) => [['tools/game-check.cjs'], ['tools/e2e/run.cjs', '--site', site]];
...
// 1. read-only name check: every project whose name contains 'claude-quest', every page
function existingNames() {
  const names = new Set();
  let next = null;
  for (let page = 0; page < 50; page++) {
    const args = ['project', 'ls', '--filter', 'claude-quest', '--format', 'json', '--non-interactive', ...(next ? ['--next', String(next)] : [])];
...
function pickName() {
  ...
  console.log(`  scope ${scope}; projects matching 'claude-quest': ${names.size ? [...names].join(', ') : 'none'}`);
  if (taken.length === NAMES.length) throw new Error(`both ${NAMES.join(' and ')} exist in ${scope}: aborting (never overwrite a project)`);
...
function get(url) {
  return new Promise((resolve, reject) => {
    const req = https.get(url, { headers: { 'user-agent': 'claude-quest-deploy-check' } }, (res) => {
      let body = '';
...
async function main() {
  ...
  if (!fs.existsSync(VERCEL)) throw new Error(`the Vercel CLI was not found on PATH (set VERCEL_BIN to its path): ${VERCEL}`);
  ...
  const gateCmds = [...(SKIP_GATES ? [] : GATES), ['tools/site.cjs', '--site', site], ...(SKIP_GATES ? [] : AFTER_SITE(site))];
  const vercelCmds = [
  ...
  if (DRY) {
    console.log('\nA real run would now run, in order (nothing below was run):');
    for (const g of gateCmds) console.log(`  ${show(NODE, g)}`);
    ...
    console.log(`\nDRY RUN: read-only name check only; no project created, nothing deployed. Would deploy ${name} to ${site}.`);
    return;
  }
