...
const FPS = 24; // default frame rate for a timeline that declares none (FILM.TIMELINE.fps)
...
function normalizeTimeline(tl) {
  ...
  const fps = tl.fps != null && Number(tl.fps) > 0 ? Number(tl.fps) : FPS;
  return { raw: tl, shots: out, duration, hasDuration: tl.duration != null, bpm: tl.bpm, cues: tl.cues, fps };
}
 ...
 */
/**
 * Asset prelude, loaded after lib.js and before the timeline when present:
 *   src/photos.js      generated by tools/photos.cjs from assets/cut
 *   src/clips.js       generated by tools/clips.cjs from assets/clips.json (instrument mode); the
 *                      fixtures carry their own at tools/fixtures/clips.js
 *   src/props.js       the shared doodle props (photo-doodle mode)
 *   src/instrument.js  the eyepiece / porthole view, flicker lists, tall pans, titles (instrument mode)
 *   src/cast.js        the film's recurring characters
 * A zero-asset film simply has none of them.
 */
// v2 (game mode): src/manifest.js, src/crt.js, every src/game/*.js (sorted) and src/foley.js load here
// too, so the game engine, the display and the TV foley exist before the timeline and the scenes.
function preludeFiles(base = SRC) {
  const clips = base !== SRC && fs.existsSync(path.join(base, 'clips.js')) ? path.join(base, 'clips.js') : path.join(SRC, 'clips.js');
  const gameDir = path.join(SRC, 'game');
  const game = base === SRC && fs.existsSync(gameDir)
    ? fs.readdirSync(gameDir).filter((f) => f.endsWith('.js')).sort().map((f) => path.join(gameDir, f))
    : [];
  const v2 = base === SRC ? [path.join(SRC, 'manifest.js'), path.join(SRC, 'crt.js'), ...game, path.join(SRC, 'foley.js')] : [];
  return [path.join(SRC, 'photos.js'), clips, ...['props.js', 'instrument.js', 'cast.js'].map((f) => path.join(SRC, f)), ...v2].filter((f) => fs.existsSync(f));
}

/**
 * The game page's sources (docs/game-spec.md 2.3): core, lib, manifest, crt, every src/game/*.js in
 * sorted order, timeline, music, shell. No scenes, no foley, no film player; every file must exist.
 */
function gameSources() {
  const gameDir = path.join(SRC, 'game');
  const game = fs.existsSync(gameDir) ? fs.readdirSync(gameDir).filter((f) => f.endsWith('.js')).sort().map((f) => path.join(gameDir, f)) : [];
  if (!game.length) die('src/game has no .js files; the game page needs the engine.');
  const files = [
    path.join(SRC, 'core.js'),
    path.join(SRC, 'lib.js'),
    path.join(SRC, 'manifest.js'),
    path.join(SRC, 'crt.js'),
    ...game,
    path.join(SRC, 'timeline.js'),
    path.join(SRC, 'music.js'),
    path.join(SRC, 'shell.js'),
  ];
...
function sources({ fixtures = false, only = null, player = true, needMusic = false, lenient = false, game = false } = {}) {
  if (game) return gameSources();
  ...
  if (only) {
    ...
    if (player) files.push(...['player.js', 'play.js'].map((f) => path.join(SRC, f)).filter((f) => f.endsWith('player.js') || fs.existsSync(f)));
  }
  const musicFile = path.join(base, 'music.js');
  return {
    files,
...
window.__h = {
  ...
  // v2 display: the TV's mode ('crt' | 'clean') and what actually ran
  crtMode(mode) {
  ...
  // v2 display: the proof layers ({ input, caption } booleans), and what they cover
  crtOverlays(o) {
    const c = window.FILM && FILM.crt;
    if (!c) return null;
  ...
  // async: clip frames the shots at T declare are decoded first (FILM.prepareFrame), so every tool
  // draws the same complete frame whatever order it asks for them in
  async render(T, o) {
    if (FILM.prepareFrame) await FILM.prepareFrame(T);
    FILM.errors = [];
  ...
  // Console-palette audit (check 8): the allowed colours once, then the current frame. A v2 film (a
  // console frame buffer, FILM.native) is audited there, every pixel: the CRT output is the TV and may
  // hold any colour. Otherwise (or with output=true) every 8th pixel of each row of the output canvas,
  // the row's phase shifting by one so all 8 columns of an 8x8 game pixel are sampled.
  nesInit(list) {
    this._nes = new Set(list.map((h) => parseInt(String(h).replace('#', ''), 16)));
  },
  nesAudit(output) {
  ...
  // Widest channel spread of a canvas (every 4th pixel): <= 6 is one flat colour. native=true reads
  // the console frame buffer (the game's picture) instead of the TV output.
  spread(native) {
    const nb = native && FILM.native ? FILM.native() : null;
    const c = nb ? nb.canvas : FILM.canvas;
    const d = (nb ? nb.ctx : FILM.ctx).getImageData(0, 0, c.width, c.height).data;
  ...
  // Luminance statistics of the TV output (check 11): mean and max over the frame, the mean of a
  // centre box of the given half-size (px), and the scanline modulation: the strongest DFT amplitude
  // (relative to the mean) of the centre band's vertical luma profile at a period within 12% of
  // pitch px (the raster's own pitch is a little under H / 180 inside the glass).
  lumaStats(box, pitch) {
    const c = FILM.canvas;
    const w = c.width, h = c.height;
...
// WebGL in the tool browser. Default: SwiftShader, pinned explicitly, so every machine renders and
// hashes the same pixels through the same CPU rasteriser whatever GPU it has (the CRT is WebGL2).
// FILM_GL=gpu uses the platform GPU through ANGLE instead: much faster previews and the real-time
// timing numbers, but its pixels are that GPU's (determinism holds within one backend, not across).
const GL_ARGS = process.env.FILM_GL === 'gpu' ? ['--ignore-gpu-blocklist', '--enable-gpu-rasterization'] : ['--use-angle=swiftshader', '--enable-unsafe-swiftshader'];
if (process.env.FILM_GL === 'gpu' && process.platform === 'darwin') GL_ARGS.push('--use-angle=metal');
...
async function launch(extraArgs = []) {
  ...
  return chromium.launch({ args: ['--allow-file-access-from-files', '--disable-background-timer-throttling', ...GL_ARGS, ...extraArgs] });
}
...
// Photographs and clip stills are the one sanctioned media, and only through the doors that put them
// there: the generated src/photos.js and src/clips.js sections, and core's own loader lines marked photo-io. A marker copied into
...
function scanForbidden(html) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (head !== null) {
      // generated asset sections: photos, and clips (the project's own or the fixtures')
      inPhotos = head === 'src/photos.js' || head === 'src/clips.js' || head === 'tools/fixtures/clips.js';
...
// AAC for every export: the AudioToolbox encoder when this ffmpeg has it (macOS: clean highs, stereo),
// otherwise ffmpeg's native encoder in mono (its stereo coding smears the NES pulse channels).
const aacCache = new Map();
function aacArgs(ffmpeg = 'ffmpeg', kbps = 192) {
  if (!aacCache.has(ffmpeg)) {
    const r = require('child_process').spawnSync(ffmpeg, ['-hide_banner', '-encoders'], { encoding: 'utf8' });
    aacCache.set(ffmpeg, /\baac_at\b/.test(String(r.stdout || '')));
  }
  return aacCache.get(ffmpeg) ? ['-c:a', 'aac_at', '-b:a', `${kbps}k`, '-ac', '2'] : ['-c:a', 'aac', '-b:a', `${kbps}k`, '-ac', '1'];
}
