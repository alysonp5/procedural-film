...
// SITE defaults to https://claude-quest-nu.vercel.app, the live production host.
'use strict';
...
const DEFAULT_SITE = 'https://claude-quest-nu.vercel.app';
const KB = 1024;
const BUDGET = { rawFail: 900 * KB, rawWarn: 750 * KB, brFail: 240 * KB, brWarn: 200 * KB };

// 12.3, verbatim; SITE is the only variable
const SHARE_TEMPLATE = [
  '<meta property="og:type" content="website">',
  '<meta property="og:title" content="CLAUDE QUEST">',
  '<meta property="og:description" content="An NES-style platformer starring Claw\'d. Play it in your browser. An unofficial fan project, not affiliated with or endorsed by Anthropic.">',
...
function shareBlock(site) {
  if (!SITE_RE.test(site)) throw new Error(`--site must match ${SITE_RE} (got ${JSON.stringify(site)})`);
  return SHARE_TEMPLATE.split('SITE').join(site);
}
...
async function renderImages(log) {
  ...
  try {
    ...
    const shot = await page.evaluate(() => {
      ...
      const tokens = { '1-1': 0, '1-2': 0, '1-3': 0, '1-4': 0 };
      const save = { v: 1, hi: 0, reach: '1-1', cleared: false, tokens,
        settings: { feel: 'modern', crt: 'on', vol: 8, mute: false, input: false, flash: 'full', keys: CQ.menu.DEFAULT_KEYS } };
      const nb = FILM.native();
      const view = { reducedFlash: false };
      t.draw(nb.ctx, view);
      CQ.menu.draw(nb.ctx, CQ.menu.init('title', { save }), { top: t.state().top }, save, { saveOff: false });
      ...
      return { mt: t.state().mt, mode: t.mode, w: nb.w, h: nb.h, px, bg: hex(L.NES[0x0f]), nes: L.NES.map(hex), idleS: sprite('clawdS_idle'), idleB: sprite('clawdB_idle') };
    });
    ...
    if (idleS.width !== 16 || idleS.height !== 16) throw new Error(`clawdS_idle is ${idleS.width}x${idleS.height}, want 16x16`);
    if (idleB.width !== 24 || idleB.height !== 24) throw new Error(`clawdB_idle is ${idleB.width}x${idleB.height}, want 24x24`);
...
// ------------------------------------------------------------------------------------------------ main
async function site({ site: siteUrl = DEFAULT_SITE, quiet = false } = {}) {
...
module.exports = { site, shareBlock, cspMeta, inlineScripts, finishPage, SITE_RE, DEFAULT_SITE, VERCEL_JSON, SHARE_TEMPLATE, BUDGET };

if (require.main === module) {
  const args = C.parseArgs(process.argv.slice(2), ['quiet']);
  const siteUrl = typeof args.site === 'string' ? args.site : DEFAULT_SITE;
  if (!SITE_RE.test(siteUrl)) {
    console.error(`[FAIL] --site must match ${SITE_RE} (got ${JSON.stringify(siteUrl)})`);
    process.exit(1);
  }
