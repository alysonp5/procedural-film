...
//                  (core, lib, timeline, scenes, music, player, and in a v2 film crt, manifest, foley, game/*, shell);
...
//   4 timeline     coverage, ids, transitions; warns on off-grid hits and cuts, a bpm whose 8ths
//                  miss the frame grid, a hit or cut that falls between frames, and a duration that is not whole bars
//   5 draw         every checked frame draws without throwing, is not one flat colour, and paints no
//                  text below the safe area (measured where it lands, so computed positions count). A v2
//                  film measures flatness where the viewer's picture is: the console frame buffer
//                  (FILM.native) while the TV shows it, the TV output (dark glass) while the set is off
//   4 timeline     shots cover 0..duration with no gaps or overlaps; every shot's file registers its id;
//                  when FILM.game exists, FILM.TIMELINE.duration * 60 === FILM.game.sim().length
...
//   NES gate (src only; the fixtures are not a console film):
//   7 palette      FILM.lib.NES is the 64-entry console palette (anchors $22 = #5C94FC, $0F = #000000) and every
//                  lib.pal value is one of its colours
//   8 pixels       every pixel of the console frame buffer (FILM.native, a v2 film) of the draw frames and every
//                  swept frame is a lib.NES colour; tolerates nothing. The CRT output is the TV, not the console,
//                  so it is not audited (without FILM.native: every 8th output pixel, as v1)
//   9 sprites      every SPRITES_DEF sprite (each legend variant) uses at most 3 non-transparent colours
//  10 audio        src/music.js (comments stripped) builds no convolver, compressor, delay or panner nodes
//                  (music.js only: the TV foley in foley.js is the one sound allowed outside the console)
//  11 display      (v2, FILM.crt) the tool browser ran the webgl2 backend; the power-on opens on the wide shot of
//                  the set and gameplay is on the full tube; the input display shows (and only) inside its
//                  windows; the closing caption lights the dark glass; the TV is dark glass before the
//                  power-on and after the power-off, lit with scanline structure mid-film, a bright dot at the
//                  collapse; clean mode (FILM.crt.mode = 'clean') paints only console colours
//                  The determinism set (2) also covers the power-on and power-off key frames (FILM.crt.marks()).
//
// Options: --scale s (default 1; the pixel check needs 1), --sweep N (every Nth frame, default fps/6: 10 at 60 fps),
//          --det N (add N evenly spaced determinism frames on top of the per-shot ones, default 0; never fewer than
//          every shot), --budget ms (fail if the slowest swept frame exceeds this; default: warn above one frame
//          period, 1000/fps ms, the live player's budget)
'use strict';
...
// A v2 film (a console frame buffer and a TV) is measured where the viewer's picture is: the console
// frame buffer while the TV shows it, the TV output (the dark glass) while the set is off.
function flatness(pg, T, native) {
  return pg.page.evaluate(async ([t, nb]) => {
    await window.__h.render(t, { post: false }); // the grain post hides a blank frame behind its own noise
    const F = window.FILM;
    const lit = !nb || !(F.crt && typeof F.crt.power === 'function') || F.crt.power(Math.floor(t * F.FPS + 1e-6)).lit;
    return { spread: window.__h.spread(nb && lit), what: nb ? (lit ? 'console frame buffer' : 'TV output (set off)') : '' };
  }, [T, !!native]);
}
...
async function main() {
  ...
  const FPS = TL.fps;
  const sweepStep = Math.max(1, Math.floor(Number(args.sweep || Math.max(1, Math.round(FPS / 6)))));
  const warnMs = Math.min(150, 1000 / FPS); // one frame period: the live player draws every frame in real time
  const nesGate = !fixtures;
  ...
  // ---------------------------------------------------------------- 3 sources (static)
  {
    ...
    files.push(path.join(C.SRC, 'play.js')); // the playable mode (skipped when absent)
    files.push(path.join(C.SRC, 'instrument.js')); // instrument mode's shared drawing code (skipped when absent)
    if (!fixtures) {
      // v2: the display, the sprite manifest, the TV foley and the game engine draw or sound too
      for (const f of ['crt.js', 'manifest.js', 'foley.js', 'shell.js']) files.push(path.join(C.SRC, f));
      const gameDir = path.join(C.SRC, 'game');
      if (exists(gameDir)) for (const f of fs.readdirSync(gameDir).filter((x) => x.endsWith('.js')).sort()) files.push(path.join(gameDir, f));
    }
    const hits = [];
  ...
  // ---------------------------------------------------------------- 10 audio (static, NES gate)
  // The console's sound chip has two pulse channels, a triangle, noise and DPCM, and nothing after
  // them: no reverb, no compressor, no echo line, no stereo field.
  if (nesGate) {
    const AUDIO_BANNED = [
    ...
    if (!mf) {
      report(10, 'audio', false, `${src.label}/music.js is missing: nothing to hold to the console sound chip`);
    } else {
      const hits = [];
      stripComments(fs.readFileSync(mf, 'utf8')).split('\n').forEach((line, i) => {
        for (const b of AUDIO_BANNED) {
          b.re.lastIndex = 0;
          if (b.re.test(line)) hits.push(`${C.rel(mf)}:${i + 1}  ${b.name}  | ${line.trim().slice(0, 100)}`);
        }
      });
      report(
        10,
        'audio',
        hits.length === 0,
        hits.length ? `${hits.length} non-console audio node(s) in ${C.rel(mf)} (the NES has no reverb, compressor, delay or panner)` : `${C.rel(mf)} builds no convolver, compressor, delay or panner nodes`,
        hits
      );
    }
  }
  ...
  const offGrid = (t) => sixteenth > 0 && Math.abs(t / sixteenth - Math.round(t / sixteenth)) > 1e-3;
  {
    ...
    // 8ths must land on whole frames (FPS * 30 / bpm): 24 fps needs 360 / bpm whole, 60 fps 900 / bpm
    const eighth = TL.bpm > 0 ? (FPS * 30) / TL.bpm : 0;
    if (TL.bpm > 0 && Math.abs(eighth - Math.round(eighth)) > 1e-6) tlWarnings.push(`bpm ${TL.bpm}: 8th notes are ${eighth.toFixed(2)} frames at ${FPS} fps, not whole frames (${FPS * 30} / bpm must be whole)`);
  ...
  try {
    browserChecks: {
      ...
      // v2: the console frame buffer, the TV, the game's simulation length, the TV's power key frames
      const v2 = await pg.page.evaluate(() => {
        const F = window.FILM;
        if (!F) return null;
        const out = { native: !!(F.native && F.crt), crt: !!(F.crt && typeof F.crt.present === 'function'), marks: null, sim: null };
        if (out.crt && typeof F.crt.marks === 'function') out.marks = F.crt.marks();
        if (F.game && typeof F.game.sim === 'function') {
          try {
            const r = F.game.sim();
            out.sim = { length: r ? r.length : undefined };
          } catch (e) {
            out.sim = { error: String((e && e.message) || e) };
          }
        }
        return out;
      });
      if (v2 && v2.sim) {
        const want = TL.duration * 60;
        if (v2.sim.error) tlProblems.push(`FILM.game.sim() threw: ${v2.sim.error}`);
        else if (!(Math.abs(v2.sim.length - want) < 1e-6)) tlProblems.push(`FILM.game.sim().length is ${v2.sim.length} frames, but FILM.TIMELINE.duration * 60 is ${want}: keep timeline.js in step with the simulation`);
      }
      report(
        4,
        'timeline',
        ...
        tlProblems.length
          ...
          : `${TL.shots.length} shots cover 0..${TL.duration}s with no gaps or overlaps; every shot's file registers its id${v2 && v2.sim ? `; FILM.game.sim().length ${v2.sim.length} = duration * 60` : v2 && v2.crt ? '; FILM.game not loaded yet (sim length not checked)' : ''}`,
      ...
      // ---------------------------------------------------------------- 7 palette, 9 sprites (NES gate)
      // The table is read through its own accessor (not core's) so the gate checks it independently:
      // lib.NES may be an array indexed $00-$3F or an object keyed '$22' / '22' / '0x22'.
      let nesColours = null; // the 64 table colours as #RRGGBB, when the table exists
      const pixelFails = [];
      let pixelSamples = 0;
      let pixelFrames = 0;
      const nativeAudit = !!(v2 && v2.native);
      const audit = nesGate && (nativeAudit || scale === 1);
      if (nesGate) {
        const nes = await pg.page.evaluate(() => {
          const L = window.FILM.lib;
          const N = L.NES;
          if (!N || typeof N !== 'object') return { missing: true };
          const entry = (i) => {
            const h = i.toString(16).toUpperCase().padStart(2, '0');
            for (const k of [i, '$' + h, h, '0x' + h, '$' + h.toLowerCase(), h.toLowerCase(), '0x' + h.toLowerCase()]) if (N[k] != null) return N[k];
            return null;
          };
          const entries = [];
          for (let i = 0; i < 64; i++) entries.push(entry(i));
          const count = Array.isArray(N) ? N.length : Object.keys(N).length;
          const pal = {};
          for (const k of Object.keys(L.pal)) pal[k] = L.pal[k];
          return { count, entries, pal };
        });
        const problems = [];
        if (nes.missing) {
          problems.push('FILM.lib.NES does not exist: the console palette table is missing from src/lib.js');
        } else {
          const norm = (v) => (typeof v === 'string' ? v.trim().toUpperCase() : v);
          const hex = /^#[0-9A-F]{6}$/;
          if (nes.count !== 64) problems.push(`lib.NES has ${nes.count} entries, not 64`);
          const bad = [];
          nes.entries.forEach((v, i) => {
            if (!hex.test(norm(v))) bad.push(`$${i.toString(16).toUpperCase().padStart(2, '0')}=${v}`);
          });
          if (bad.length) problems.push(`lib.NES entries that are not #RRGGBB: ${bad.slice(0, 12).join(', ')}${bad.length > 12 ? ` (+${bad.length - 12})` : ''}`);
          if (norm(nes.entries[0x22]) !== '#5C94FC') problems.push(`lib.NES $22 is ${nes.entries[0x22]}, the console sky blue is #5C94FC`);
          if (norm(nes.entries[0x0f]) !== '#000000') problems.push(`lib.NES $0F is ${nes.entries[0x0f]}, the console black is #000000`);
          nesColours = [...new Set(nes.entries.map(norm).filter((v) => hex.test(v)))];
          const set = new Set(nesColours);
          const off = Object.entries(nes.pal).filter(([, v]) => !set.has(norm(v)));
          for (const [k, v] of off) problems.push(`lib.pal.${k} = ${v} is not a lib.NES colour`);
        }
        report(
          7,
          'palette',
          problems.length === 0,
          problems.length ? `${problems.length} problem(s) with the console palette` : `lib.NES is the 64-entry console palette ($22 #5C94FC, $0F #000000) and all ${Object.keys(nes.pal).length} lib.pal values are in it`,
          problems
        );
        if (nesColours) await pg.page.evaluate((list) => window.__h.nesInit(list), nesColours);

        const spr = await pg.page.evaluate(() => {
          const SP = window.FILM.lib.SPRITES_DEF;
          if (!SP || typeof SP !== 'object') return { missing: true };
          const over = [];
          let variants = 0;
          for (const name of Object.keys(SP)) {
            ...
            legends.forEach((lg, li) => {
              ...
              if (cols.size > 3) over.push(`${name}${legends.length > 1 ? `[${li}]` : ''}: ${cols.size} colours (${[...cols].join(' ')})`);
            });
          }
          return { sprites: Object.keys(SP).length, variants, over };
        });
        if (spr.missing) report(9, 'sprites', false, 'FILM.lib.SPRITES_DEF does not exist: nothing to count');
        else report(9, 'sprites', spr.over.length === 0, spr.over.length ? `${spr.over.length} sprite(s) use more than 3 colours (a console sprite palette is 3 colours + transparent)` : `${spr.sprites} sprites (${spr.variants} legend variants) use at most 3 colours each`, spr.over);
      }
      // 8 pixels: sample the frame just drawn; every sampled colour must be in the console table
      const auditFrame = async (f, label) => {
        if (!audit || !nesColours) return;
        const a = await pg.page.evaluate(() => window.__h.nesAudit());
        pixelFrames++;
        pixelSamples += a.n;
        if (a.bad) pixelFails.push(`${label} f${f} (T=${(f / FPS).toFixed(3)}) ${a.source}: ${a.bad} of ${a.n} samples are not lib.NES colours, e.g. ${a.colours.join(', ')}`);
      };
      ...
      for (const shot of TL.shots) {
        ...
        for (const [label, f] of [['first', f0], ['middle', fm], ['last', f1]]) {
          ...
          const fl = softened ? null : await flatness(pg, f / FPS, nativeAudit);
          if (fl && fl.spread <= 6) {
            drawFails.push(`${shot.id} ${label} frame f${f} (T=${(f / FPS).toFixed(3)})${fl.what ? ' ' + fl.what : ''} is one flat colour: a blank frame reads as a bug`);
          }
      ...
      if (nesGate) {
        ...
        else if (!nesColours) report(8, 'pixels', false, 'no lib.NES table to check the rendered pixels against (see 7 palette)');
        else
          report(
            8,
            'pixels',
            pixelFails.length === 0,
            pixelFails.length
              ? `${pixelFails.length} of ${pixelFrames} frames paint colours outside lib.NES`
              : `${pixelFrames} frames (draw frames + sweep), ${pixelSamples} ${nativeAudit ? 'console frame buffer pixels (FILM.native, every pixel)' : 'samples'}: every pixel is a lib.NES colour`,
            pixelFails
          );
      }
      ...
      // the TV is WebGL: in the tool browser that is SwiftShader, a CPU rasteriser, so the frame period is
      // a warning about software GL, not about the player (a GPU draws the same frame in a few ms)
      const glInfo = v2 && v2.crt ? await pg.page.evaluate(() => window.__h.crtInfo()) : null;
      const soft = glInfo && /swiftshader/i.test(String(glInfo.renderer)) ? ' in software GL (SwiftShader; the player runs it on the GPU)' : '';
      report(
        6,
        'cost',
        sweepErr.length ? false : over ? false : max > warnMs ? 'WARN' : true,
        `${sweep.length} frames swept (every ${sweepStep}): median ${median.toFixed(0)}ms, mean ${mean.toFixed(0)}ms, max ${max.toFixed(0)}ms${budget != null ? ` (budget ${budget}ms)` : ''}${max > warnMs ? ` - above one ${FPS} fps frame (${warnMs.toFixed(1)}ms)${soft}` : ''}`,
        costDetails
      );

      // ---------------------------------------------------------------- 11 display (v2)
      // The TV: the real GL path ran, power states read as they should, the picture has scanlines, and
      // clean mode is a pure nearest-neighbour upscale of console colours.
      if (nesGate && v2 && v2.crt) {
        const fails = [];
        const notes = [];
        const Hh = pg.info.h;
        ...
        // the camera: the whole set before the power-on, the full tube during play
        ...
        if (!(Math.abs(cams[1].zoom - 1) < 1e-9)) fails.push(`f${mid}: gameplay should fill the frame with the tube (camera zoom ${cams[1].zoom.toFixed(3)})`);
        notes.push(`camera: zoom ${cams[0] ? cams[0].zoom.toFixed(2) : '-'} at the power-on start, ${cams[1].zoom.toFixed(2)} mid-film`);
        await setOv({ caption: false });
        const lit = await stats(mid, 'mid-film');
        if (!(lit.mean > 0.12)) fails.push(`f${mid} mid-film: the TV output is dark (mean luma ${pct(lit.mean)})`);
        if (pitch) {
          const sc = lit.scan;
        ...
        if (M.powerOn) {
          ...
          const ln = await stats(o.line + 3, 'power-on line');
          if (!(ln.max > 0.85 && ln.mean < 0.15)) fails.push(`f${o.line + 3}: expected the bright horizontal line on dark glass (max ${pct(ln.max)}, mean ${pct(ln.mean)})`);
        }
        if (M.powerOff) {
          const o = M.powerOff;
          const dot = await stats(o.dot + 2, 'power-off dot');
          if (!(dot.max > 0.85 && dot.mean < 0.1)) fails.push(`f${o.dot + 2}: expected the collapse dot on dark glass (max ${pct(dot.max)}, mean ${pct(dot.mean)})`);
          if (o.end + 5 < total) {
        ...
        // the input display: drawn inside a window, absent outside it
        if (ov0.input && ov0.input.length) {
          ...
          fails.push('FILM.game exists but FILM.crt.inputWindows() found no stretch of play');
        }
        ...
        if (ov0.caption) {
          ...
          notes.push('closing caption: no room after the power-off (FILM.crt.captionWindow() is null; the film needs >= 30 frames after powerOff start + 90)');
        }
        ...
        report(
          ...
          fails.length === 0,
          fails.length
            ? `${fails.length} problem(s) with the TV`
            : `backend ${info.backend} (${info.renderer}); the set in its dark room before the power-on and after the power-off, the line and the dot, the full tube lit mid-film with scanlines${ov0.input && ov0.input.length ? `, the input display in ${ov0.input.length} window(s)` : ''}${ov0.caption ? ', the closing caption' : ''}; clean mode paints only console colours; median frame ${median.toFixed(0)}ms at ${pg.info.w}x${Hh}`,
          [...fails, ...notes]
        );
      }
      ...
      // v2: the TV's power sequences are seeded noise and eased geometry; hold them to the same bar
      let tvFrames = 0;
      if (v2 && v2.marks) {
        const n0 = candidates.length;
        const o = v2.marks.powerOn;
        if (o) candidates.push(o.click, o.line + 3, o.open + 4, o.open + 20, o.open + 40, o.locked, o.settled - 10);
        const x = v2.marks.powerOff;
        if (x) candidates.push(x.start + 4, x.line + 3, x.dot + 2, x.afterglow + 20);
        tvFrames = candidates.length - n0;
      }
      ...
      report(
        ...
        mism.length
          ...
          : detErrors.length
            ...
            : `${frames.length} frames (first, middle, last of all ${TL.shots.length} shots${transitions ? `, ${transitions} transition(s)` : ''}${tvFrames ? `, ${tvFrames} TV power frames` : ''}${detExtra ? `, +${detExtra}` : ''}) hash identically: cold, sequential, warm forward, warm reverse, fresh shuffled with decoys (${((Date.now() - tDet) / 1000).toFixed(1)}s)`,
