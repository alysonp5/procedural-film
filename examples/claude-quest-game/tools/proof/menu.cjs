...
//
// Drives CQ.menu.step (src/game/50-menu.js) through every screen with button edges and asserts the
...
// Then paints every screen into a recording canvas and checks every colour it uses is a lib.NES colour.
'use strict';

const path = require('path');
const H = require('./harness.cjs');

const G = H.load({ files: [path.join(H.SRC, 'lib.js')] });
const FILM = G.FILM;
const CQ = G.CQ;
const M = CQ.menu;
const B = CQ.BUTTONS;
...
function save(over) {
  const s = {
    v: 1, hi: 12345, reach: '1-1', cleared: false, tokens: { '1-1': 0, '1-2': 0, '1-3': 0, '1-4': 0 },
...
// ------------------------------------------------------------------------------------------ title
{
  ...
  check('title rows', it.map((r) => r.label).join(',') === 'NEW QUEST,CONTINUE,OPTIONS' && it[1].disabled === true, it.map((r) => r.label + (r.disabled ? '(off)' : '')).join(','));
  check('title starts on NEW QUEST', row(s, env) === 'new');
  ...
  check('START on NEW QUEST: new', r.actions.length === 1 && r.actions[0].type === 'new' && r.sounds[0] === 'select', JSON.stringify(r.actions));
  r = press(s, [B.A], env);
  check('A on NEW QUEST: new', r.actions[0] && r.actions[0].type === 'new');
...
// ------------------------------------------------------------------------------------------ labels
{
  ...
  const glyphs = new Set(Object.keys(FILM.lib.FONT));
...
// ------------------------------------------------------------------------------------------ painting
{
  const NES = new Set(FILM.lib.NES.map((c) => c.toUpperCase()));
  const used = new Set();
  let draws = 0;
  function fakeCtx(c) {
    const g = {
  ...
  const snap = { top: 4200, name: 'THE STACK', world: '1-2', tokens: 5 };
  const screens = [
    M.init('title', env),
    M.init('options', env),
  ...
  check('every colour painted is a lib.NES colour', off.length === 0 && draws > 0, off.length ? off.join(' ') : `${used.size} colours, ${draws} fills`);
}
...
// ------------------------------------------------------------------------------------------ purity
{
  ...
  check('CQ.menu is frozen', Object.isFrozen(M));
}
