...
// (EN1 to EN14). Owner: P1. Small, purpose-built maps in the GAME_DEFS schema (docs/game-spec.md 4.3),
...
// en-corner: blocks in row 6 for head grazes: B at 10, * at 20, + at 30, B at 40
...
// en-sky: the same blocks in a sky level (items stay where they sprout)
const SKY = blank(40);
put(SKY, 10, 7, 'N');
put(SKY, 14, 7, 'M');
put(SKY, 18, 7, '*');
...
// en-e1: a short run to the pole (col 16), the castle at 22; next en-e2
const E1 = blank(40);
put(E1, 16, 3, '!');
for (let r = 4; r <= 8; r++) put(E1, 16, r, '|');
put(E1, 16, 9, 'X');

// en-e2: a keep: a lava pit behind the start (cols 2-3), the start platform (4-7), the bridge (8-19)
// over lava, the chain (19) and the ENTER key (22) on its platform (20-22), Pearl's room with Pearl at 32
const E2 = [
  //         1         2         3
...
const DEFS = {
  ...
  'en-under': { id: 'en-under', name: 'UNDER', world: '8-4', kind: 'under', rows: done(blank(40, 'G')), time: 400, song: 'underground',
    start: { x: 40, y: 148 } },
  'en-sky': { id: 'en-sky', name: 'SKY', world: '8-5', kind: 'sky', rows: done(SKY), time: 400, song: 'sky',
    start: { x: 40, y: 148 } },
  ...
  'en-e2': { id: 'en-e2', name: 'THE KEEP', world: '9-2', kind: 'castle', rows: E2, time: 300, song: 'castle',
    start: { x: 72, y: 116 }, bridge: [8, 19], axe: 22 },
  // the keep with a boss on its bridge and the boss song (the seam's boss fields)
  'en-boss': { id: 'en-boss', name: 'THE BOSS', world: '9-3', kind: 'castle', rows: E2, time: 300, song: 'castle',
    start: { x: 72, y: 116 }, bridge: [8, 19], axe: 22, boss: { tx: 14, range: [9, 18], hp: 5 }, bossSong: 'boss' },
};
