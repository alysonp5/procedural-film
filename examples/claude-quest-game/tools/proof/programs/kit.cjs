// tools/proof/programs/kit.cjs : the level bots' vocabulary (docs/game-spec.md 13.2). Owner: P3.
//
// A level program is a list of moves a player means ("hop onto h4", "the terminal on the ?M? block",
// "across the lava lake on the lift"), each a { until(W, t), btn(W, t) } move in the shape of CQ.moves
// (src/game/20-tape.js), built from the engine's own planner: CQ.moves and CQ.goals, with o.free: true on
// every planner move (docs/game-spec.md D13: the film's style rules do not bind the game's proofs), and
// the planner's eye, CQ.lookahead (a copy of the world played forward with CQ.step). A move holds closure
// state, so a program is built fresh for every run: kit(CQ) is called once per run.
//
...
//   k.fight(name, done)          Claw'd Code against the Big Bug: carets while it stays safe, else dodge
//   k.dodge(name, done)          stand, step aside or hop clear, whichever stays unhurt
//   k.goto, k.wait, k.hold, k.mode, k.skipIf, k.move ...   CQ.moves' plain moves
// Every move carries a label for the runner's failure report (the stuck move). KIT_DEBUG=1 logs every
// search decision.
'use strict';

module.exports = function kit(CQ) {
  const M = CQ.moves, g = CQ.goals;
  const BT = CQ.BUTTONS;
  ...
  const K = {
    CQ, M, g, BT, A, B, START, DOWN, LEFT, RIGHT, UP, R, RB, L, LB, rowTop, NS,
  ...
  // fight(name, done, x): Claw'd Code against the Big Bug: every frame, throw a caret (a fresh B press,
  // facing right, fewer than two in flight) when throwing and then standing stays unhurt, else the first
  // dodge that does (the dodging rules below; x.keep: where he may stand), until done(W). A hop is played
  // through to its landing.
  function fight(name, done, x = {}) {
    let hop = null, hi = 0;
    const flying = (W) => (W.carets || []).filter((c) => c.st === 'fly').length;
    const look = x.look || 50;
    const safeSeq = (W, seq) =>
      CQ.lookahead(W, seq, look, (c, ev, k) => {
        if (hazardOf(ev) || c.p.st === 'dead' || c.p.y > 176 || (x.keep && c.p.ground && !x.keep(c))) return false;
    ...
    return label(name, {
      ...
      btn(W) {
        ...
        if (W.p.st !== 'play') return 0;
        if (W.p.face < 0) return R; // turn to face the boss
        if (W.p.code && !(W.prev & B) && flying(W) < 2 && safeSeq(W, [B, 0])) return B;
        const d = dodgeSeq(W, look, x.keep, x.only);
        if (d[0] & A) {
          hop = d;
          hi = 1;
        }
        ...
        if (W.p.ground && W.p.st === 'play' && !(W.prev & A)) {
          for (const xj of xs) {
            ...
            for (const n of ns) {
              ...
              seq.push(after);
              if (CQ.lookahead(W, seq, horizon + pre.length, (cc, ev, k) => goal(cc, ev, k - pre.length), O)) {
                plan = seq;
                i = 1;
                return seq[0];
              }
  ...
  function beamSearch(W, goal, x = {}) {
    ...
    const survives = (c) => {
      const q = CQ.cloneWorld(c);
      for (let k = 0; k < after; k++) if (lost(q, CQ.step(q, hold))) return false;
      return true;
    };
    const steady = (c) => {
      if (!c.p.ground) return true; // (only a pipe or the pole leaves him off his feet here)
      const q = CQ.cloneWorld(c);
      for (let k = 0; k < 8; k++) if (lost(q, CQ.step(q, 0))) return false;
      return q.p.ground || q.p.st !== 'play';
    };
    let frontier = [{ c: CQ.cloneWorld(W), seq: [], cut: 0 }];
  ...
  function dodgeSeq(W, look, keep, only) {
    ...
    for (const seq of DODGES) {
      ...
      let life = 0;
      const ok = CQ.lookahead(W, seq, look, (c, ev, k) => {
  ...
  // planner's own (CQ.lookahead with o.free). done(W) is tested whenever no plan is running. x.b: the
  ...
  function advance(name, done, x = {}) {
    ...
    function tryOut(W, seq, horizon, judge) {
      let life = 0;
      const ok = CQ.lookahead(W, seq, horizon, (c, ev, k) => {
