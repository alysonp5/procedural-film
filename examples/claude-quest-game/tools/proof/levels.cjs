...
//
// A program (tools/proof/programs/*.cjs, one per file or an array of them) is a list of planner moves built from CQ.moves and CQ.goals
...
function capOf(G, prog) {
  if (prog.cap) return typeof prog.cap === 'function' ? prog.cap(G.CQ) : prog.cap;
  const def = G.CQ.GAME_DEFS[prog.opts.start];
...
function plan(G, prog, feel, o = {}) {
  ...
  for (let f = 0; f < cap; f++) {
    const b = f === 0 ? CQ.BUTTONS.START : bot() & 255;
    tape.push(b);
    const ev = con.step(b);
    for (const e of ev) {
      events.push(e);
    ...
    if (ex === prog.exit.length) {
      ...
      const bad = prog.check ? prog.check(W, events, CQ).filter((c) => !c[0]) : [];
...
function summary(W, CQ) {
  return `score ${W.score}, lives ${W.lives}, tokens ${CQ.tokenCount(W)}, retries ${W.retries | 0}`;
}
    ...
    for (const feel of feels) {
      ...
      if (replan) {
        ...
        const obj = {
          ...
          meta: { program: prog.name, what: prog.what || '', score: r.score, tokens: G.CQ.tokenCount(r.W), lives: r.W.lives, planned: 'tools/proof/levels.cjs --replan' },
        };
        ...
        console.log(`[PASS] ${tag} planned ${r.frames} frames, fp ${r.fp}, ${summary(r.W, G.CQ)} (${((Date.now() - s0) / 1000).toFixed(1)} s)`);
      } else {
        ...
        console.log(`[PASS] ${tag} replayed ${r.frames} frames, fp ${r.fp}, ${summary(r.W, G.CQ)}`);
      }
    }
  }
