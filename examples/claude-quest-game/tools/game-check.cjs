...
//       mode), plus the title, options, controls, pause, continue, clear, credits and demo screens;
...
//       CQ.NAMES.length < 1000 afterwards.
...
//       step + snapshot + drawSnap on the 1-3 tape (p95 <= 4 ms, p99 <= 8 ms); 60 +/- 0.5 steps per
...
function g1() {
  ...
  if (!index) probs.push('web/index.html is missing (run tools/site.cjs)');
    ...
    site = m ? m[1] : null;
    if (!site || !S.SITE_RE.test(site)) probs.push(`og:url names ${site}, which does not match ${S.SITE_RE}`);
    else {
      try {
    ...
    const order = ['<meta charset="utf-8">', '<meta http-equiv="Content-Security-Policy"', '<meta name="viewport"', '<title>CLAUDE QUEST</title>',
...
async function g2g3(browser, url) {
  ...
  try {
    ...
    for (const f of files) {
      ...
      if (/^full\./.test(f)) {
        // the credits roll on live input after the tape's last frame
        await t.steps(240);
        await auditNow();
      }
    }
    ...
    await t.page.evaluate(() => FILM.shell.proof.feed({ opts: { start: '1-1', feel: 'modern' }, rle: '08:1,00:5j,08:1,00:a' }));
    await t.until(() => FILM.shell.stats.state === 'paused' && !FILM.shell.proof.feeding, null, 10000);
    await t.steps(5);
    await auditNow();
    // continue: one life, walk into the first bug
    await t.page.evaluate(() => FILM.shell.proof.feed({ opts: { start: '1-1', feel: 'modern', lives: 1 }, rle: '08:1,00:5j,80:ct' }));
    ...
    names = await t.page.evaluate(() => FILM.__cq.NAMES.length);
  ...
  report('G3', 'no console errors or CSP violations; CQ.NAMES < 1000', okG3, `${g3Errors.length} error(s)/violation(s) over the whole G2 run (both full tapes included); CQ.NAMES.length ${names}`, g3Errors);
}
...
async function g4(url) {
  ...
  try {
    ...
    try {
      await t.goto('?proof=1&clean=1');
      await t.title();
      // step + snapshot + drawSnap over the 1-3 tape, in one task (the console's draw is snapshot + drawSnap)
      const tape13 = E.tape('1-3.modern');
      const cost = await t.page.evaluate((tp) => {
        const CQ = FILM.__cq;
        for (let i = 0; i < 400 && CQ.warmStep(64) !== true; i++);
        const runs = [];
        const dec = (rle) => {
          const out = [];
      ...
      if (p95 > 4 || p99 > 8) probs.push(`step+draw p95 ${p95.toFixed(2)} ms / p99 ${p99.toFixed(2)} ms over the 1-3 tape (limits 4 / 8)`);
      lines.push(`step + snapshot + drawSnap over 1-3 (${cost.length} frames): p50 ${pct(cost, 50).toFixed(2)} ms, p95 ${p95.toFixed(2)} ms, p99 ${p99.toFixed(2)} ms, max ${Math.max(...cost).toFixed(2)} ms (limits 4 / 8)`);
      // step rate in play
      await t.key('Enter');
      ...
      if (m1 !== 'credits') probs.push(`the full tape replay ended in ${m1}`);
