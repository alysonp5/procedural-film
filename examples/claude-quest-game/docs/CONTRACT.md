...
A film of **Claw'd — the Claude Code mascot — playing through a Super-Mario-Bros-style pixel platformer
level to rescue Princess Pearl**, drawn entirely by JavaScript on a canvas, with music and sound effects
synthesised in JavaScript.
It ships as one self-contained HTML file and as a rendered MP4 for YouTube (landscape).
...
3. **Stateless per frame.** A scene's `draw` may not depend on a previous frame having been drawn. Caches are allowed only if they are pure functions of their inputs.
4. **Canvas** is 1920 wide by 1080 tall (landscape, declared as `width`/`height` in `FILM.TIMELINE`). Output is 24 fps.
