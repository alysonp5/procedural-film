...
//
// The game (docs/game-spec.md section 10) adds, live only: the songs death, gameover, sky, boss and hurry;
// fast(id), the hurry-up variant of a level song at speed minus 1 (overworldFast, undergroundFast,
// skyFast, castleFast, bossFast); its effects (pause, unpause, hurt, checkpoint, token, throw, zap, poof,
// bosshit, bossdefeat, spit, ember, spring, crumble and the menu's cursor, select, continue, back); and
// the driver pause: a `pause` event holds the song's frame and silences it while effects play on, and
// `unpause` resumes the same frame. The film emits none of these, so its soundtrack is byte-identical.
//
...
(function () {
  ...
  const CHORD = {
    ...
    Em: [[4, 7, 11], 40],
    Em7: [[4, 7, 11, 2], 40],
    // the game's keys: A major (sky), C minor (boss)
    'C#m': [[1, 4, 8], 49],
    E: [[4, 8, 11], 40],
    E7: [[4, 8, 11, 2], 40],
    Bm7: [[11, 2, 6, 9], 47],
  ...
  const halfChords = (bar) => bar.split(' ').map((c) => [c, bar.includes(' ') ? 8 : 16]);
  // The game's helpers. Pulse-2 arpeggios: each chord broken up and back down in 16ths, from its root at
  // or above `lo`: root, third, fifth, octave, the third above, and down again.
  function arps(chords, lo) {
    return chords
      .map((bar) =>
        halfChords(bar)
          .map(([c, n]) => {
            ...
            while (pc(r) !== pcs[0]) r++;
            const tones = [r];
            for (const p of pcs.slice(1, 3)) {
              let m = tones[tones.length - 1] + 1;
              while (pc(m) !== p) m++;
              tones.push(m);
            }
            tones.push(r + 12, tones[1] + 12);
            const pat = [0, 1, 2, 3, 4, 3, 2, 1];
            return Array.from({ length: n }, (_, k) => nn(tones[pat[k % 8]])).join(' ');
          })
          .join(' ')
      )
      .join(' ');
  }
  // A walking triangle bass in quarter notes: root, third, fifth, then a half step below the next bar's
  // root (a bar of two chords walks root, fifth, root, approach).
  function walk(chords) {
    return chords
      .map((bar, i) => {
        const hs = halfChords(bar).map(([c]) => CHORD[c]);
        const next = CHORD[halfChords(chords[(i + 1) % chords.length])[0][0]][1];
        const [pcs, r1] = hs[0];
        const third = r1 + pc(pcs[1] - pcs[0]);
        const q = hs.length === 1 ? [r1, third, r1 + 7, next - 1] : [r1, r1 + 7, hs[1][1], next - 1];
        return q.map((m) => nn(m) + ':4').join(' ');
      })
      .join(' ');
  }
  // Off-beat stabs in 8ths, the chord's third and fifth in turn, voiced from D4 up.
  function offbeats(chords) {
    return chords
      .map((bar) =>
        halfChords(bar)
          .map(([c, n]) => {
            ...
            const t = voice(pcs[1], 62);
            const f = voice(pcs[2], 62);
            return Array.from({ length: n / 4 }, (_, k) => `r:2 ${k & 1 ? f : t}:2`).join(' ');
          })
          .join(' ')
      )
      .join(' ');
  }
  // The boss's triangle ostinato, one bar per chord in 8ths: root twice, the third, root, then a
  // chromatic climb from the fourth to the fifth.
  function ostinato(chords) {
    return chords
      .map((c) => {
        const [pcs, r] = CHORD[c];
        const third = pc(pcs[1] - pcs[0]);
        return [0, 0, third, 0, 5, 0, 6, 7].map((d) => nn(r + d) + ':2').join(' ');
      })
      .join(' ');
  }
  ...
  // THE CLOUD (A major). A: the tune climbs the tonic triad from the fifth below and floats down;
  // B: a rising sequence, then higher still over D, D minor and Bm, and the E7 turn home.
  const SKY = {
    lead: [
      'E5:2 A5:2 C#6:4 B5:2 A5:2 E5:4', // A
      'G#5:4 E5:2 C#5:2 E5:6 r:2', // C#m
      'F#5:2 A5:2 D6:4 C#6:2 B5:2 A5:4', // D
      'B5:6 G#5:2 E5:8', // E
      'C#6:2 A5:2 F#5:4 A5:2 C#6:2 E6:4', // F#m
      'F#6:6 E6:2 D6:4 A5:4', // D
      'D6:4 C#6:2 B5:2 G#5:4 A5:2 B5:2', // Bm7 | E
      'A5:12 r:4', // A
      'A5:3 B5 A5:2 F#5:2 D5:4 r:2 A5 B5', // D
      'B5:3 C#6 B5:2 G#5:2 E5:4 r:2 B5 C#6', // E
      'E6:4 C#6:2 B5:2 G#5:4 C#6:4', // C#m
      'A5:6 G#5:2 F#5:8', // F#m
      'F#5:2 A5:2 D6:3 E6 F#6:4 E6:2 D6:2', // D
      'F6:6 E6:2 D6:4 A5:4', // Dm
      'B5:2 D6:2 F#6:4 E6:2 D6:2 B5:4', // Bm
      'G#5:2 B5:2 E6:4 D6:2 B5:2 G#5:4', // E7
    ],
    chords: ['A', 'C#m', 'D', 'E', 'F#m', 'D', 'Bm7 E', 'A', 'D', 'E', 'C#m', 'F#m', 'D', 'Dm', 'Bm', 'E7'],
  };
  // The Big Bug (C minor): two bars of ostinato, eight of tune, two of stop-time (the loop skips the intro).
  const BOSS = {
    chords: ['Cm', 'G', 'Cm', 'Ab', 'Fm', 'G', 'Cm', 'Ab', 'Db', 'G', 'Cm', 'G'],
    lead: [
      'G5:4 D#5:2 C5:2 G5:3 G#5 G5:2 F5:2', // Cm
      'D#5:6 C5:2 G#4:4 C5:2 D#5:2', // Ab
      'F5:4 G#5:2 C6:2 C#6:3 C6 G#5:2 F5:2', // Fm
      'G5:6 F5 D#5 D5:4 B4:4', // G
      'C6:4 G5:2 D#5:2 C6:3 D6 D#6:2 D6:2', // Cm
      'C6:6 G#5:2 D#5:4 F5:2 G5:2', // Ab
      'G#5:4 F5:2 C#5:2 G#5:3 A#5 G#5:2 F5:2', // Db
      'G5:4 B5:2 D6:2 F6:4 D6:2 B5:2', // G
      'C6:2 r:2 C6 r C6:2 r:2 D#6:2 D6:2 C6:2', // Cm, stop-time
      'B5:2 r:2 B5 r B5:2 r:2 G5:2 G#5:2 B5:2', // G
    ],
  };
  // the credits' parade: the ending-full tune's first seven bars (its eighth, the tonic, is the card's)
  const CREDITS = {
    lead: HOOK.concat([HOOK[0], 'D5:3 E5 G5:4 r:2 G5 A5 B5:2 A5:2', 'G5:4 A#5:4 A5:4 G5:2 E5:2']),
    chords: ['D', 'Bm', 'Gmaj7', 'A', 'D', 'G', 'Gm'],
  };
  ...
  const SONG_DEFS = {
    ...
    // ---- the game (live only; the film never requests these)
    // Claw'd is hit: a D6-A5 yelp, silence while he hangs (the dead state hops at frame 30, row 5), then a
    // falling D harmonic minor line lands on the tonic with a timpani and dies away by frame 144, inside the
    // 180-frame dead state.
    death: {
      speed: 6,
      loop: false,
      p1: '@leadHi D6 A5:2 r:2 @lead A#5:2 A5 G5 F5:2 E5 C#5:2 @fade D5:10',
      p2: '@harm F5 D5:2 r:2 G5:2 F5 E5 D5:2 C#5 A4:2 @fadeSoft F4:10',
      t: '@hold D3:2 r:3 G2:4 A2:5 @ring D3:8 r:2',
      d: 'r:5 A:9 D:10',
    },
    // game over: one slow bar of D minor sinking home, the last chord an open fifth (D, A, D) that fades
    // out by frame 176, inside the 180-frame game over
    gameover: {
      speed: 11,
      loop: false,
      p1: '@sing F5:3 E5 D5:2 C#5:2 D5:2 A#4:2 @fade A4:4',
      p2: '@soft D5:3 C#5 A4:2 A4:2 F4:2 G4:2 @fadeSoft D4:4',
      t: '@hold D3:4 A2:4 D3:2 G2:2 D3:4',
      d: 'D:4 A:4 r:4 D:4',
    },
    // THE CLOUD, A major: a floating vibrato lead over pulse-2 arpeggios, a walking triangle bass, light
    // off-beat hats and a sparse kick. A (row 0) states the tune; B (row 128) climbs higher and turns
    // through a borrowed D minor before the E7 brings it home. 16 bars, 25.6 s, looping whole.
    sky: {
      speed: 6,
      sections: { A: 0, B: 128 },
      p1: '@air ' + bars(SKY.lead),
      p2: '@arp ' + arps(SKY.chords, 57),
      t: '@bass ' + walk(SKY.chords),
      n: '[r:2 h:2 r:2 h:2 r:2 h:2 r:2 h:2]8 [r:2 h h r:2 h h r:2 h h r:2 h h]7 r:2 h h r:2 h h r:2 o:2 o:2 o:2',
      d: '[K:16 K:8 S:8]4 [K:16 K:8 S:8]3 K:16 K:4 S:4 S:2 S:2 S S S S',
    },
    // the Big Bug, C minor: a kick on every beat, a triangle ostinato climbing to the fifth, off-beat
    // stabs on pulse 1 and the lead on pulse 2, so the roar (pulse 1) never takes the tune. Two bars of
    // ostinato, eight of tune (with a Neapolitan D-flat before the dominant), two of stop-time; the loop
    // returns after the intro. 12 bars, 16 s.
    boss: {
      speed: 5,
      p1: '@stab ' + offbeats(BOSS.chords.slice(0, 2)) + ' | ' + offbeats(BOSS.chords.slice(2)),
      p2: '@brass r:32 | ' + bars(BOSS.lead),
      t: '@bass ' + ostinato(BOSS.chords.slice(0, 2)) + ' | ' + ostinato(BOSS.chords.slice(2)),
      n: '[h:2]16 | [h:2 h:2 s:2 h:2 h:2 h:2 s:2 h:2]10',
      d: '[K:4]8 | [K:4]36 K:4 K:4 K:4 S:2 S:2',
    },
    // the credits: the ending's eight bars as a parade march at the overworld's tempo (speed 6, 96 frames a
    // bar). Bars 1-7 are the cast's parade (672 frames, the engine's CREDITS_ROLL; fireworks land on the
    // downbeats of bars 2, 4, 6 and 7); bar 8's tonic lands on the frame the final card appears (section
    // 'card', where a START skip starts it) and the chord rings down to a whisper. No loop.
    credits: {
      speed: 6,
      loop: false,
      sections: { card: 112 },
      p1: '@lead ' + bars(CREDITS.lead) + ' @singEnd D5:64',
      p2: '@stab ' + comp(CREDITS.chords) + ' @softEnd F#4:64',
      t: '@bass ' + bassLine(CREDITS.chords) + ' @ring D3:16 r:48',
      n: `[${HATS}]7 x:16 r:48`,
      d: `[${GROOVE}]6 K:4 S:2 K:2 S:2 S:2 S S S S D:16 r:48`,
    },
    // hurry up: three rising stabs up an A7 chord and a held, shaking A, exactly 90 frames
    hurry: {
      speed: 5,
      loop: false,
      p1: '@hit C#5:2 r E5:2 r G5:2 r @alarm A5:9',
      p2: '@hit A4:2 r C#5:2 r E5:2 r @alarm2 E5:9',
      t: '@hold A2:2 r A2:2 r A2:2 r @ring A2:8 r',
      n: 'h:3 h:3 h:3 x:9',
      d: 'K:3 K:3 K:3 L:9',
    },
  };
  ...
  // fast(id): the hurry-up variant of a level song, the same definition one frame a row faster (speed
  // minus 1). Sections are rows, so they scale with it; nothing is recomposed.
  function fast(id) {
    const def = SONG_DEFS[id];
    return compile(id + 'Fast', Object.assign({}, def, { speed: def.speed - 1 }));
  }
  const FAST = ['overworld', 'underground', 'sky', 'castle', 'boss'];
  for (const id of FAST) SONGS[id + 'Fast'] = fast(id);
  ...
  const SFX = {
    ...
    // ---- the game (live only; the film emits none of these). Every note sequence is original.
    // pause: E6 up a fifth to a ringing B6, echoed softly below a frame late; plays over the silenced song
    pause: () => ({
      pri: 6,
      p1: cat(pt(mhz('E6'), 4, { d: 2, v: [12, 11] }), pt(mhz('B6'), 16, { d: 2, v: [12, 0], vib: [0.004, 5] })),
      p2: cat(gap(1), pt(mhz('B5'), 3, { d: 1, v: [7, 7] }), pt(mhz('E6'), 16, { d: 1, v: [8, 0] })),
    }),
    unpause: () => ({ pri: 6, p1: pt(mhz('E6'), 8, { d: 2, v: [11, 0] }) }),
    // hurt: the shrink keeps the film's pipe slurp (the film emitted `pipe` for it)
    hurt: (e) => SFX.pipe(e),
    // checkpoint: a B major arpeggio gliding up to a ringing B6
    checkpoint: () => ({
      pri: 3,
      p2: cat(...['B5', 'D#6', 'F#6'].map((n, i) => pt(mhz(n), 3, { d: 1, v: [11, 11], trig: i === 0 })), pt(mhz('B6'), 15, { d: 1, v: [12, 0], vib: [0.005, 5], trig: false })),
    }),
    // token: D7 and B6 trading every two frames, sparkling out (a minor third, never the coin's fourth)
    token: () => ({ pri: 3, p2: Array.from({ length: 20 }, (_, k) => ({ v: Math.max(1, Math.round(12 - k * 0.55)), d: k < 4 ? 2 : 1, t: ptimer(mhz((k >> 1) & 1 ? 'B6' : 'D7')), trig: k === 0 })) }),
    // throw: a caret leaves the hand, an octave swept up in 8 frames
    throw: () => ({ pri: 1, p2: pt(700, 8, { d: 1, v: [14, 4], bend: Math.pow(2, 1 / 7) }) }),
    // zap: a caret kill, a gritty crunch over a quick triangle drop
    zap: () => ({ pri: 2, n: nz([[5, 14], [7, 12], [4, 11], [8, 9], [6, 8], [9, 6], [7, 5], [10, 3], [9, 2], [11, 1]]), t: tthud(260, 7, 0.8) }),
    // poof: a caret on a wall
    poof: () => ({ pri: 0, n: nz([[6, 12], [8, 9], [9, 6], [10, 3]]) }),
    // bosshit: a caret on the Big Bug, a heavy thud on the triangle and noise (pulse 1 stays with the roar)
    bosshit: () => ({
      pri: 3,
      t: tthud(170, 16, 0.92),
      n: nz([[3, 15, 1], [6, 14], [9, 12], [10, 11], [11, 9], [11, 8], [12, 7], [12, 6], [12, 5], [13, 4], [13, 3], [13, 3], [14, 2], [14, 2], [14, 1], [15, 1]]),
    }),
    // bossdefeat: a long falling roar, a growl sinking 330 -> 50 Hz (a different fall from the film's
    // bossfall, which the caret kill skips) over a swelling then fading rumble
    bossdefeat: () => {
      const n = 64;
      const growl = [];
      for (let k = 0; k < n; k++) {
        const env = k < 4 ? 10 + k : Math.round((13 * (n - 1 - k)) / (n - 5));
        growl.push({ v: Math.max(0, env - (k & 1) * 2), d: k < 8 ? 1 : 0, t: ptimer(330 * Math.pow(50 / 330, k / (n - 1)) * (k & 1 ? 0.93 : 1.07)), trig: k === 0 });
      }
      const rumble = Array.from({ length: n }, (_, k) => [k < 16 ? 11 : k < 40 ? 12 : 13, (k < 6 ? 5 + 2 * k : 15 - (k - 6) * 0.25) - (k & 2 ? 2 : 0)]);
      return { pri: 5, p2: growl, n: nz(rumble) };
    },
    // spit: the boss's roar spit, a hiss
    spit: () => ({ pri: 1, n: nz([[3, 9], [2, 13], [2, 14], [3, 13], [2, 12], [3, 11], [3, 10], [4, 8], [4, 6], [5, 4], [5, 3], [6, 1]]) }),
    // ember: a lava ember leaping, a sizzle
    ember: () => ({ pri: 0, n: nz([[3, 10], [2, 5], [3, 9], [2, 4], [4, 8], [3, 3], [4, 5], [5, 2]]) }),
    // spring: a boing, a wobbling sweep up; held A launches higher and longer
    spring: (e) =>
      e.held
        ? { pri: 2, p2: pt(262, 12, { d: 1, v: [15, 6], bend: Math.pow(4, 1 / 11), vib: [0.08, 3] }) }
        : { pri: 2, p2: pt(330, 6, { d: 1, v: [14, 6], bend: Math.pow(2.5, 1 / 5), vib: [0.08, 3] }) },
    // crumble: a falling lift starts to shake, a rattle
    crumble: () => ({ pri: 1, n: nz([[7, 12], [10, 5], [8, 11], [10, 4], [8, 10], [11, 3], [9, 8], [11, 2], [9, 5], [12, 1]]) }),
    // the menus (the shell sends cursor, select and back straight to the audio; the continue screen emits
    // cursor and continue)
    cursor: () => ({ pri: 1, p2: pt(mhz('A6'), 3, { d: 2, v: [9, 3] }) }),
    // select: a major third up, E6 to a ringing G#6; back mirrors it down
    select: () => ({ pri: 2, p2: cat(pt(mhz('E6'), 3, { d: 1, v: [11, 11] }), pt(mhz('G#6'), 7, { d: 1, v: [11, 0], trig: false })) }),
    continue: () => SFX.select(),
    back: () => ({ pri: 1, p2: cat(pt(mhz('G#6'), 2, { d: 1, v: [9, 9] }), pt(mhz('E6'), 4, { d: 1, v: [8, 0], trig: false })) }),
  };
