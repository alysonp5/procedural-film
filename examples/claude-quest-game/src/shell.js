 ...
 * tablets, and a polite live region for screen readers. It forks the proven parts of src/play.js (the
 * key merge, the latch, suppress, the gamepad poll, the touch pad, live sound); play.js and player.js
 * stay the film page's.
 *
 ...
 * CQ.clock (src/game/45-clock.js), advanced only by the requestAnimationFrame stamp. Every animation
 ...
 * (localStorage 'claude-quest.save.v1') is derived here from the console's events.
 *
...
(function () {
  ...
  if (!FILM || !FILM.game || typeof FILM.game.create !== 'function' || !FILM.__cq || !FILM.__cq.clock || !FILM.__cq.menu) return;
  const CQ = FILM.__cq;
  const MENU = CQ.menu;
  ...
  const SAVE_KEY = 'claude-quest.save.v1';
  const TOAST_FRAMES = 150;
  const CHROME_H = 28;
  // shown on the title, the menus and the demo: this is a fan game, not an Anthropic product
  const LEGAL = 'UNOFFICIAL FAN GAME \u00b7 NOT AFFILIATED WITH OR ENDORSED BY ANTHROPIC';
  ...
  const LEVELS = () => (Array.isArray(CQ.GAME_ORDER) && CQ.GAME_ORDER.length ? CQ.GAME_ORDER.slice() : ['1-1', '1-2', '1-3', '1-4']);
  ...
  const S = {
    ...
    demo: null,
    demoT: 0,
    menu: null, // CQ.menu state (title and paused)
    clock: CQ.clock.make(),
    fast: 0,
    // input
    ...
    dirty: true,
    warmed: false,
    warmN: 24, // names CQ.warmStep builds this animation frame (warm())
    lastStamp: null,
    // audio
    actx: null,
    gain: null,
  ...
  function defaults() {
    const tokens = {};
    for (const id of ['1-1', '1-2', '1-3', '1-4']) tokens[id] = 0;
    return {
      v: 1, hi: 0, reach: '1-1', cleared: false, tokens,
  ...
  const CSS = [
    ...
    '#note.on{opacity:1}',
    '#note.gate{color:#e8e0cf;animation:cq-blink 1.2s steps(1,end) infinite}',
    '@keyframes cq-blink{0%{opacity:1}62%{opacity:.25}}',
  ...
  function buildUI() {
    ...
    canvas.setAttribute('aria-label', 'Claude Quest game screen');
    canvas.tabIndex = -1;
    ui.canvas = canvas;
  ...
  function unlock() {
    ...
    try {
      ...
      S.snd = null;
      if (window.console) console.error('claude-quest: audio failed to start', e);
    }
    updateNote();
  }
  ...
  function updateHelp() {
    ...
    let items;
    if (S.state === 'demo') {
      items = [['ANY KEY', 'MENU'], ['F', 'FULL'], ['C', 'CRT'], ['M', 'MUTE'], ['', LEGAL]];
      ...
      items = [[move, 'MOVE'], [lab('A'), 'JUMP'], [lab('B'), 'RUN/THROW'], [lab('START') + '/P', 'PAUSE'], ['F', 'FULL'], ['C', 'CRT'], ['M', 'MUTE']];
    } else {
      items = [choose, [lab('A') + '/' + lab('START'), 'OK'], [lab('B'), 'BACK'], ['F', 'FULL'], ['C', 'CRT'], ['M', 'MUTE'], ['', LEGAL]];
    }
  ...
  function levelName(id) {
    const d = CQ.GAME_DEFS && CQ.GAME_DEFS[id];
  ...
  function startGame(start, opts) {
    ...
    S.state = 'game';
    S.menu = null;
    S.first = true; // NEW QUEST sends START on the console's first step
    S.suppress = rawHeld();
    S.lastSent = 0;
    S.runBest = 0;
    S.pauseReq = null;
  ...
  function present(nb, clean) {
    ...
    try {
      ...
      if (!presentErr && window.console) console.error('claude-quest: the CRT failed, presenting clean', e);
      presentErr = true;
      crt.mode = 'clean';
      FILM.presentNearest(nb.canvas, FILM.ctx);
    }
  ...
  // The title builds every sprite variant ahead of play (CQ.warmStep, 11.1 step 5): 24 names an animation
  ...
  function warm(stamp) {
    ...
    S.lastStamp = stamp;
    if (S.warmed || S.state !== 'title' || typeof CQ.warmStep !== 'function') return;
    if (gap > 3 * CQ.clock.STEP_MS) S.warmN = Math.max(1, S.warmN >> 1);
    else if (gap > 0 && gap < 1.5 * CQ.clock.STEP_MS && S.warmN < WARM_MAX) S.warmN++;
    S.warmed = CQ.warmStep(S.warmN) === true;
  }
  ...
  function frame(stamp) {
    ...
    pollPads();
    warm(stamp);
    const n = S.fast > 0 ? S.fast : CQ.clock.advance(S.clock, stamp);
  ...
  function onVisible() {
    CQ.clock.reset(S.clock); // no catch-up for the time away
  ...
  function feed(tape) {
    ...
    const opts = Object.assign({ start: '1-1', feel: save.settings.feel, top: save.hi, shellMenu: true }, tape.opts || {});
  ...
  if (PROOF) {
    shell.proof = Object.freeze({
      ...
      fast(n) {
        ...
        if (!S.fast) CQ.clock.reset(S.clock);
        return S.fast;
      },
      idle(frames) {
