...
(function () {
  ...
  // v2: the console's frame buffer. The game draws every pixel of this 320x180 canvas each frame
  // (it is never cleared for it), then the display (src/crt.js, or presentNearest) scales it to the
  // output. One buffer for the whole page: a frame is a pure function of the frame index because
  // the game repaints all of it.
  const NATIVE_W = 320;
  const NATIVE_H = 180;
  let nativeBuf = null;
  FILM.NATIVE_W = NATIVE_W;
  FILM.NATIVE_H = NATIVE_H;
  FILM.native = function native() {
    if (!nativeBuf) {
      const canvas = makeCanvas(NATIVE_W, NATIVE_H);
      const ctx = canvas.getContext('2d', { willReadFrequently: true });
      ctx.imageSmoothingEnabled = false;
      nativeBuf = { canvas, ctx, w: NATIVE_W, h: NATIVE_H };
    }
    return nativeBuf;
  };
  /** The clean display: whole-pixel nearest-neighbour upscale of the native buffer to fill ctx's canvas. */
  FILM.presentNearest = function presentNearest(src, ctx) {
    const c = ctx.canvas;
    ctx.save();
