 ...
 *
 *   FILM.crt.present(srcCanvas, outCtx, { frame, T })  fill outCtx.canvas with the TV image
 ...
 *                      the controller input display (FILM.game.sim().states[f].btn) during play, and the
...
(function () {
  ...
  crt.caption = crt.caption || ['EVERY FRAME WAS PLAYED', 'BY A GAME ENGINE', 'FROM RECORDED CONTROLLER INPUT \u00b7 NOTHING IS KEYFRAMED'];
  crt.backend = null;
  ...
  /** The closing caption's frames: after the dot dies, to the film's end (or FILM.TIMELINE.crt.caption). */
  crt.captionWindow = function captionWindow() {
    const c = FILM.TIMELINE && FILM.TIMELINE.crt;
    if (c && Array.isArray(c.caption)) return [c.caption[0], c.caption[1]];
    const { off } = windows();
    if (!off) return null;
  ...
  // The input display shows while the player is in control: from each world's first playable frame to
  // its goal (the flagpole grab; in the castle the Enter key, the bridge or the boss's fall).
  let winCache = null;
  crt.inputWindows = function inputWindows() {
    const G = FILM.game;
    if (!G || typeof G.sim !== 'function') return [];
    let S;
    try {
      S = G.sim();
    } catch (e) {
    ...
    if (winCache && winCache.S === S) return winCache.w;
    const ev = S.events || [];
    const first = (type, from, to) => {
      for (const e of ev) if (e.type === type && e.f >= from && e.f < to) return e.f;
      return -1;
    };
    const PLAY = 2;
    const out = [];
    for (let f = 0; f < S.length; ) {
      while (f < S.length && S.states[f].m !== PLAY) f++;
      if (f >= S.length) break;
      const a = f;
      const world = S.states[f].world;
      while (f < S.length && S.states[f].m === PLAY && S.states[f].world === world) f++;
      let b = f;
      for (const type of ['flagpole', 'axe', 'bridge', 'bossfall']) {
        const e = first(type, a, f);
        if (e >= 0) {
          b = e;
          break;
        }
      }
      // never past the last button the tape holds in this stretch (a scripted walk shows no pad), nor
      // into the power-off
      let lastBtn = -1;
      for (let k = a; k < f; k++) if (S.states[k].btn) lastBtn = k;
      if (lastBtn < 0) continue;
      b = Math.min(b, lastBtn + 45);
      const { off } = windows();
  ...
  function buildLut() {
    const p = crt.params;
    const key = [p.sharp, p.chroma, p.artifacts, p.lumaSigma, FILM.lib && FILM.lib.NES ? 1 : 0].join('|');
...
void main() {
  ...
  function drawPad(g, W, H, btn, a, f) {
    const u = (H / 1080) * 1.15;
    const B = (FILM.game && FILM.game.BUTTONS) || { A: 1, B: 2, SELECT: 4, START: 8, UP: 16, DOWN: 32, LEFT: 64, RIGHT: 128 };
  ...
  function btnAt(f) {
    try {
      const S = FILM.game.sim();
      const st = S.states[Math.max(0, Math.min(S.length - 1, f))];
      return st ? st.btn | 0 : 0;
    } catch (e) {
      return 0;
    }
  }
