...
run(FFMPEG, [
  ...
  ...C.aacArgs(FFMPEG, 192), '-movflags', '+faststart', '-t', String(dur), OUT,
]);
const ph = cfg.phone || 720;
const phH = Math.round((ph * H) / W / 2) * 2;
run(FFMPEG, ['-v', 'error', '-y', '-i', OUT, '-vf', `scale=${ph}:${phH}`, '-c:v', 'libx264', '-crf', '23', '-preset', 'medium', ...C.aacArgs(FFMPEG, 128), PHONE]);
