...
//   --scale s       render scale (default 1 = the timeline's frame; 2 = 4K for claude-quest)
...
async function main() {
  ...
  try {
    ...
    const tv = await pg.page.evaluate((m) => window.__h.crtMode(m), args.clean ? 'clean' : null);
    if (tv) console.log(`display ${tv.mode}`);
