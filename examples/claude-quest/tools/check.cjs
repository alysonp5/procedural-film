...
//   8 pixels       every sampled pixel (every 8th pixel of each row, the row's phase shifting so all 8 columns of a
//                  game pixel are hit) of the draw frames and every swept frame is a lib.NES colour; tolerates nothing
...
function flatness(pg, T) {
  return pg.page.evaluate(async (t) => {
  ...
  try {
    browserChecks: {
      ...
      const auditFrame = async (f, label) => {
        ...
        pixelFrames++;
        pixelSamples += a.n;
        if (a.bad) pixelFails.push(`${label} f${f} (T=${(f / FPS).toFixed(3)}): ${a.bad} of ${a.n} samples are not lib.NES colours, e.g. ${a.colours.join(', ')}`);
      };
        ...
        if (!audit) report(8, 'pixels', 'WARN', `skipped: the pixel palette check samples exact colours and needs --scale 1 (got ${scale})`);
          ...
          report(
            ...
            pixelFails.length
              ...
              : `${pixelFrames} frames (draw frames + sweep), ${pixelSamples} samples: every pixel is a lib.NES colour`,
            pixelFails
          );
      }
      ...
      report(
        ...
        `${sweep.length} frames swept (every ${sweepStep}): median ${median.toFixed(0)}ms, mean ${mean.toFixed(0)}ms, max ${max.toFixed(0)}ms${budget != null ? ` (budget ${budget}ms)` : ''}${max > warnMs ? ` - above one ${FPS} fps frame (${warnMs.toFixed(1)}ms)` : ''}`,
        costDetails
      );
