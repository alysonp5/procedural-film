...
function preludeFiles(base = SRC) {
  ...
  return [path.join(SRC, 'photos.js'), clips, ...['props.js', 'instrument.js', 'cast.js'].map((f) => path.join(SRC, f))].filter((f) => fs.existsSync(f));
}
...
window.__h = {
  ...
  // Console-palette audit (check 8): the allowed colours once, then every 8th pixel of each row of the
  // current frame, the row's phase shifting by one so all 8 columns of an 8x8 game pixel are sampled.
  nesInit(list) {
  ...
  nesAudit() {
    ...
    return { n, bad, colours: [...seen].map(([v, at]) => '#' + v.toString(16).toUpperCase().padStart(6, '0') + ' at ' + at) };
  },
  hash() {
    const c = FILM.canvas;
